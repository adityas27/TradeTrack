...
const TradeList = () => {
  ...
  const handleStatusUpdate = (tradeId, newStatus) => {
    fetch(`http://127.0.0.1:8000/api/trades/trades/${tradeId}/update-status/`, {
      ...
      .then((res) => res.json())
      .then((updatedTrade) => {
        // Optional: manually update if websocket is delayed
        setTrades((prev) =>
          prev.map((t) => (t.id === updatedTrade.id ? updatedTrade : t))
        );
      })
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <h2 className="text-3xl font-extrabold text-gray-800 mb-6">📈 Live Trades</h2>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {trades.map((trade) => (
                <tr key={trade.id} className="hover:bg-gray-50 transition-colors duration-150 ease-in-out">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                    {trade.trader_username || "N/A"} {/* Assuming trader object has a username or similar field */}
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                    ...
                    {/* Assuming approved_by object has a username */}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
